...
module.exports = class RegisterSlashCommand extends BaseSlashCommand {
     ...
     async run(client, interaction) {
          ...
          let alreadyRegistred_message = new EmbedBuilder()
               ...
               .setDescription(
                    `This username has already registered, ${checkUser?.username} !`
               )
               .setColor("Red")
               .setTimestamp()
               .setFooter({
          ...
          const successLogged_message = new EmbedBuilder()
               ...
               .setDescription(
                    `${interaction.user.username} you have been registered Successfully!`
               )
               .setColor("Green")
               .setTimestamp()
               .setFooter({
          ...
          const notMatchPassword_message = new EmbedBuilder()
               ...
               .setDescription(
                    "Password doesn't match, Please confirm your password!"
               )
               .setColor("Yellow")
               .setTimestamp()
               .setFooter({
          ...
          if (
               checkUser?.username === interaction.options.get("username").value
          ) {
               return interaction.reply({
                    embeds: [alreadyRegistred_message],
                    ephemeral: true
               });
          }
          ...
          if (checkIdUser?.user_id === interaction.user.id) {
               return interaction.reply({
                    embeds: [AlreadyUserRegistred_message],
                    ephemeral: true
               });
          }
          if (
               interaction.options.get("password").value !==
               interaction.options.get("confirm").value
          ) {
               return interaction.reply({
                    embeds: [notMatchPassword_message],
                    ephemeral: true
               });
          }
          try {
