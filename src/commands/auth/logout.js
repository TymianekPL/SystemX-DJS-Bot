...
module.exports = class LogoutSlashCommand extends BaseSlashCommand {
     ...
     async run(client, interaction) {
          const getUser = await RegisterModel.findOne({user_id: interaction.user.id})
          ...
          try {
               if (getUser?.isLogged) {
               const LogoutUser = await RegisterModel.findOneAndUpdate({user_id: interaction.user.id}, {isLogged: false})
               LogoutUser.save();
               return interaction.reply({embeds: [success_message],ephemeral: true})
               } else {
                    return interaction.reply({embeds: [alreadyLoggedOut_message],ephemeral: true})
               }
          } catch(err) {
               console.log(err)
          }
